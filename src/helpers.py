...
def create_text_file(resource: Path) -> Generator[TextIO, None, None]:
    try:
        ...
        message = f"{__name__}.create_text_file({str(resource):s})"
        raise HelpersException(message) from e
...
def read_text_file(resource: Path) -> Generator[TextIO, None, None]:
    try:
        ...
        message = f"{__name__}.read_text_file({str(resource):s})"
        raise HelpersException(message) from e


def read_json(resource: Path) -> dict[str, Any]:
...
def erase_directory_contents(directory: Path) -> None:
    ...
    if not directory.exists():
        ...
        raise HelpersException(message)

    if not directory.is_dir():
        message = f"The path '{directory}' is not a directory."
        raise HelpersException(message)
       ...
       for item in directory.iterdir():
           try:
               ...
               message = f"{__name__}.erase_directory_contents({directory}) {item}"
               raise HelpersException(message) from e
