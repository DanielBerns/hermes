...
class SampleBuilder:
    def __init__(
        ...
        self._points_of_sale_rows_processor: RowsProcessor = (
            points_of_sale_rows_processor
        )
        self._selector: RowsSelector = selector
        self._articles_by_point_of_sale_rows_processor: RowsProcessor = (
            articles_by_point_of_sale_rows_processor
        )
    ...
    def points_of_sale(self) -> Generator[dict[str, Any], None, None]:
        ...
        try:
            ...
            logger.warning(
                f"{self.__class__.__name__}.points_of_sale: ScraperStop raised"
            )
    ...
    def articles_by_point_of_sale(self) -> Generator[dict[str, Any], None, None]:
        ...
        for this_point_of_sale in self.table:
            if self.selector.select(this_point_of_sale):
                for this_article in self.scraper.articles_by_point_of_sale(
                    this_point_of_sale[Sample.POINT_OF_SALE_CODE]
                ):
                    processed_article = (
                        self.articles_by_point_of_sale_rows_processor.execute(
                            this_article
                        )
                    )
