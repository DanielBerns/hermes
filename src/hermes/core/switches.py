...
class Switches:
    ...
    def __init__(self, identifier: str) -> None:
        ...
        self._line_regex = re.compile(
            r"^\s*\d+\.\s*(?P<key>.+?)\s*#\s*(?P<value>True|False)\s*$"
        )
            ...
            for line_number, line in enumerate(text, 1):
                ...
                if line.startswith("# "):
                    ...
                    if not current_group:
                        raise SwitchesException(
                            f"Invalid group name at line {line_number}: '{line}'"
                        )
                    # self.table[current_group] = {}
                else:  # Otherwise, it should be a component line.
                    if not current_group:
                        raise SwitchesException(
                            f"Component found before any group definition at line {line_number}: '{line}'"
                        )

                    match = line_regex.match(line)
                    if not match:
                        raise SwitchesException(
                            f"Malformed component line at {line_number}: '{line}'"
                        )
