...
class Services:
    def __init__(
        ...
        self.info_root = get_directory(Path.home() / INFO)
        self.secrets_root = get_directory(Path.home() / SECRETS)
        USE_CLI = False
        if CLI_ENABLED:
            self.cli = CLI(
                description="Process data from precios_claros@mecon for a given region."
            )
            self.configure_cli()
            arguments = self.cli.arguments
        else:
            arguments = {
                MESSAGE_BOARD_KEY: "no",
                INSTANCE_KEY: "Patagonia",
                DATABASE_NAME_KEY: "mecon_dev"
            }
        self.message_board = arguments.get(MESSAGE_BOARD_KEY) == YES
